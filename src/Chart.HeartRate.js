			...
			helpers.each(data.datasets,function(dataset){
				...
				helpers.each(dataset.data,function(dataPoint,index){
					...
					datasetObject.points.push(new this.PointClass({
						value : dataPoint,
						label : data.labels[index].split("|")[0],
						...
						yIndex: (this.options.scaleUse2Y && datasetIdx != 0 ) ? 1 : 0,
                        zoneColor: data.labels[index].split("|")[1]
					}));
				},this);
		...
		addData : function(valuesArray,label){
			...
			helpers.each(valuesArray,function(value,datasetIndex){
				...
				this.datasets[datasetIndex].points.push(new this.PointClass({
					value : value,
					label : label.split("|")[0],
					datasetLabel: this.datasets[datasetIndex].label.split('|')[0],
					x: this.scale.calculateX(this.scale.valuesCount+1),
					y: this.scale.endPoint,
					strokeColor : this.datasets[datasetIndex].pointStrokeColor,
					fillColor : this.datasets[datasetIndex].pointColor,
					yIndex: (this.options.scaleUse2Y && datasetIndex !== 0 ) ? 1 : 0,
                    zoneColor: this.datasets[datasetIndex].label.split('|')[1]
                }));
			},this);
			...
			helpers.each(this.datasets,function(dataset){
                ...
                helpers.each(pointsWithValues, function(point, index){
                    if (index === 0){
                        ...
                        ctx.save();
                        ctx.beginPath();
                        ctx.strokeStyle = point.zoneColor;
                        ctx.moveTo(point.x, point.y); 
                        ctx.bezierCurveTo(
                            previous.controlPoints.outer.x,
                                previous.controlPoints.outer.y,
                        ...
                        ctx.lineTo(pointsWithValues[index - 1].x, pointsWithValues[index - 1].y);
                        ctx.fillStyle = point.zoneColor;
                        ctx.fill();
                        ctx.moveTo(point.x, point.y); 
                        //ctx.stroke();
                        ctx.restore();
